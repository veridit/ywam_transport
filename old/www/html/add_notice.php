<?
	...
	if(isset($_POST["action"])	&& $_POST["action"]=="addnotice"){
		...
		$sSQL="UPDATE  tbl_special_notice SET notice_date = '".date('Y-m-d h:i:s')."', user_id = ".$_SESSION["User_ID"].", notice_title = '".mysql_real_escape_string(addslashes($_POST["txttitle"]))."', notice = '".mysql_real_escape_string(addslashes($_POST["txtinfo"]))."' WHERE notice_id= 1";
		//print($sSQL);
