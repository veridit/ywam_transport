...
?>
	<select name="<?=$sNAME?>" style="width:<?=$sWIDTH?>px;" size="<?=$sSIZE?>">
		<option value=""><?=$sOPTIONAL?></option>
		<?	for($iCOUNTER=2011;$iCOUNTER<=2025;$iCOUNTER++){?>
...
function fn_PRINT_TRIP_SLIP($iRESERVATION_ID, $bRE_PRINT = 0){
	...
	$iLAST_MILEAGE		=	0;
							...
							$sTRIP_SLIP_TEXT		=	"<table cellspacing='3' cellpadding='0' border='0' align='left'>";
							...
							$sTRIP_SLIP_TEXT		.=	"<tr><td style='width: 220px'><span style='font-size:13px; font-weight:bold;'>Reservation No:</span></td><td style='width: 315px'><span style='font-size:13px;'>".$iRESERVATION_ID."</span></td>";
							if($rowRESERV_INFO['card_no']==""){
							$sTRIP_SLIP_TEXT		.=	"<td><span style='font-size:13px; font-weight:bold;'>GAS CARD No:".str_repeat('&#95;',10)."</span></td></tr>";
							}else{
							$sTRIP_SLIP_TEXT		.=	"<td><span style='font-size:13px; font-weight:bold;'>GAS CARD No:</span>".str_repeat('&#95;',3).$rowRESERV_INFO['card_no'].str_repeat('&#95;',3)."</td></tr>";
							}
							$sTRIP_SLIP_TEXT		.=	"<tr><td><span style='font-size:13px; font-weight:bold;'>Reserved By:</span></td><td><span style='font-size:13px;'>".$rowRESERV_INFO['f_name']."&nbsp;".$rowRESERV_INFO['l_name']."</span></td>";
							...
							$sTRIP_SLIP_TEXT		.=	"<tr><td><span style='font-size:13px; font-weight:bold;'>Assigned Driver:</span></td><td><span style='font-size:13px;'>".$rowRESERV_INFO['assigned_driver']."</span></td>";
							...
							$sTRIP_SLIP_TEXT		.=	"<tr><td><span style='font-size:11px; font-weight:bold;'>whole miles only, no tenths</span></td><td colspan='2'>".str_repeat('&nbsp;',25)."<span style='font-size:11px; font-weight:bold;'>If you were given a gas card,</span>&nbsp;<span style='font-size:11px; font-weight:bold;'>circle one</span><br />";
							...
							$sTRIP_SLIP_TEXT		.=	"<tr><td colspan='3'><span style='font-size:13px; font-weight:bold;'>Describe any problems or vehicle damage</span></td></tr>";
...
/*	$sSQL	=	"SELECT end_gas_percent FROM tbl_trip_details 
					WHERE res_id = (SELECT MAX(tbl_reservations.res_id) FROM tbl_reservations 
					INNER JOIN tbl_trip_details ON tbl_reservations.res_id = tbl_trip_details.res_id WHERE vehicle_id = ".$iVEHICLE_ID.") ";
		Replaced this code as res_id does not mean oldest trip, just oldest reserved trip. A trip with a lower res_id could be on a later date.
*/
	$sSQL =  "SELECT end_gas_percent,end_mileage,tbl_trip_details.reg_date FROM tbl_trip_details 
						INNER JOIN tbl_reservations ON tbl_reservations.res_id = tbl_trip_details.res_id 
						WHERE vehicle_id = ".$iVEHICLE_ID." ORDER BY tbl_trip_details.reg_date DESC LIMIT 1";
//	print($sSQL);
...
/*	$sSQL	=	"SELECT DATE_FORMAT(reg_date, '%m/%d/%Y') AS end_gas_date FROM tbl_trip_details 
						WHERE res_id = (SELECT MAX(tbl_reservations.res_id) FROM tbl_reservations 
						INNER JOIN tbl_trip_details ON tbl_reservations.res_id = tbl_trip_details.res_id WHERE vehicle_id = ".$iVEHICLE_ID.") ";
Replaced this code as res_id does not mean oldest trip, just oldest reserved trip. A trip with a lower res_id could be on a later date.
*/

	$sSQL	=	"SELECT DATE_FORMAT(tbl_trip_details.reg_date, '%m/%d/%Y') AS end_gas_date FROM tbl_trip_details 
						INNER JOIN tbl_reservations ON tbl_reservations.res_id = tbl_trip_details.res_id 
						WHERE vehicle_id = ".$iVEHICLE_ID." ORDER BY tbl_trip_details.reg_date DESC LIMIT 1";	
	//print($sSQL);
...
function fn_DEL_DEPT($iDEPT_ID){
	...
	if(mysql_num_rows($rsUSER)>0){
		while($rowDEPT	=	mysql_fetch_array($rsUSER)){
			/* We are deleting everything and it is cuasing issues. Therefore let us delete nothing! The new code after this comment is all we are doing.
			...
			$rsDEL_TRIP	=	mysql_query($sSQL) or die(mysql_error());
			*/
			$sSQL	=	"UPDATE  tbl_user SET active=0, dept_id='6666' WHERE user_id = ".$rowDEPT['user_id'];	
			$usrDEACTIVE	=	mysql_query($sSQL) or die(mysql_error());
		
		}
	}
	/* we do not want to delete the user just deactivate them. This has been done above.
...
function fn_DEACTIVATE_EXPIRED_PERMITS(){
