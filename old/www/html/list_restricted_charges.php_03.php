<?
	...
	if(isset($_POST["action"])	&& $_POST["action"]=="search"){
		if(isset($_POST["drpdept"]) && $_POST["drpdept"]!="")			{$sDeptNo	=	$_POST["drpdept"];		$sCriteriaSQL	.=	" AND tbl_restricted_charges.dept_id = '".$sDeptNo."'";}
		if(isset($_POST["drpvehicle"]) && $_POST["drpvehicle"]!="")		{$iVehicleNo=	$_POST["drpvehicle"];	$sCriteriaSQL	.=	" AND tbl_restricted_charges.vehicle_id = ".$iVehicleNo;}
		if(isset($_POST["drpmonth"]) && $_POST["drpmonth"]!="")			{$iMonth	=	$_POST["drpmonth"];		$sCriteriaSQL	.=	" AND tbl_restricted_charges.charge_month = '".$iMonth."'";}
		if(isset($_POST["drpyear"]) && $_POST["drpyear"]!="")			{$iYear		=	$_POST["drpyear"];		$sCriteriaSQL	.=	" AND tbl_restricted_charges.charge_year = '".$iYear."'";}
		
		
		$sSQL	=	"SELECT tbl_restricted_charges.charge_id, tbl_restricted_charges.vehicle_id, tbl_restricted_charges.dept_id, ".
		"MONTHNAME(CONCAT('1970-',tbl_restricted_charges.charge_month,'-10')) AS month_name, tbl_restricted_charges.charge_year AS year, dept_name, vehicle_no, FORMAT(total_charge, 2) AS amount ".
		...
		"WHERE calc_method = 'Total Charge' ".$sCriteriaSQL." ORDER BY (tbl_vehicles.vehicle_no+0)";
		//print($sSQL);
...
<html>
<head>
<title>List Flat Rate Charges</title>
									...
									<table border="0" cellspacing="0" cellpadding="0" width="100%" height="100%">
								 		<tr>			
								  			<td class="TextObject" align="center">
								   				<h1 style="margin-bottom: 0px;">LIST FLAT RATE CHARGES</h1>
								  			</td>
								 		</tr>
									</table>
                   				</td>
				...
				<tr valign="top" align="left"><td colspan="4">&nbsp;</td></tr>
               	...
               	<tr valign="top" align="left">
                	...
                	<td width="949" class="TextObject" align="center">
						<form name="frm1" action="list_restricted_charges.php" method="post">
							...
							<table cellpadding="0" cellspacing="5" border="0" width="949" align="center" class="box">
														...
														<tr>
															...
															<td width="50%" align="right" class="label">
																<? 	if(isset($_POST["optExcelReport"]) && ($_POST["optExcelReport"]	==	"flds") && $iRECORD_COUNT>0){
																		$sFname	=	'excel_reports/list_restricted_charges.csv'; print("<a href='download.php?f=$sFname&Dir=./excel_reports/'>download excel file</a>&nbsp;&nbsp;&nbsp;");
																	}?>
															</td>
														</tr>
													</table>
								...
								<?	if($iRECORD_COUNT>0){	
										if (isset($_POST["optExcelReport"]) && $_POST["optExcelReport"]!=""){
											$fp	=	"";
											$fp = fopen($sFname, 'w');
											if($_POST["optExcelReport"]	==	"flds"){fputcsv($fp, explode(',','Vehicle_No,Dept_Name,Month,Year,Amount'));}
										}
								?>
									...
									<td>
										<table cellpadding="0" cellspacing="0" border="0" width="100%" align="center">
											<tr>
												...
												<td width="300" class="colhead">Dept. Name</td>
											...
											<?		$listed	=	0;	
													while($rowVEHICLE	=	mysql_fetch_array($rsVEHICLE)){
														if (isset($_POST["optExcelReport"]) && $_POST["optExcelReport"]!=""){
															if($_POST["optExcelReport"]	==	"flds"){ fputcsv($fp, explode(',', $rowVEHICLE["vehicle_no"].",".$rowVEHICLE["dept_name"].",".$rowVEHICLE["month_name"].",".$rowVEHICLE["year"].",".str_replace(",","",$rowVEHICLE["amount"])));}
														}
											...
											?>			<tr>
															...
															<td class="coldata" align="center"><? if(fn_USER_PERMISSIONS_TABLE($_SESSION["User_ID"], $iOPT_RESTRICTED_VEHICLE_CHARGES_MODIFY)){?><a href="edit_restricted_charges.php?id=<? echo $rowVEHICLE['charge_id'];?>">edit</a><? 	}?>&nbsp;/&nbsp;<? if(fn_USER_PERMISSIONS_TABLE($_SESSION["User_ID"], $iOPT_RESTRICTED_VEHICLE_CHARGES_DELETE)){?><a href="javascript:void(0);" onClick="if(confirm('are you sure to delete this charge?')) {fn_DELETE_CHARGES(<? echo $rowVEHICLE['charge_id'];?>);} return false;">delete</a><?	}?></td>
														</tr>
											<?			}$listed++;
													}
