<?
	...
	if(isset($_POST["txtdeptdatetime"]) && $_POST["txtdeptdatetime"]!="")	$sTimePickerDate1 = mysql_real_escape_string($_POST["txtdeptdatetime"]);
	...
	if(isset($_POST["action"])	&& $_POST["action"]!="")				$sAction		=	$_POST["action"];
			...
			if(isset($_POST["scurrentdate"]) && $_POST["scurrentdate"]!="")	$sToday_Date = mysql_real_escape_string($_POST["scurrentdate"]);	else	$sToday_Date = date("Y-m-d");
/*		if(strtotime($sToday_Date)<strtotime(date('Y-m-d'))) 		$sToday_Date		=	date("Y-m-d");	
			This has been taken out to allow the graph to go back in time. Tony Arcus 28/7/2017
*/?>
