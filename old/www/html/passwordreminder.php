<?
	...
	if(isset($_POST["action"])	&& $_POST["action"]=="passwordreminder"){
		...
		if (!isset($_POST["captcha"]) || $_POST["captcha"]==""){
			$sMessage		='<li>Please enter security image letters';
		}elseif(strtolower($_POST["captcha"])!=strtolower($_SESSION['securimage_code_value'])) {
			$sMessage		="<li class='bold-font'>Image Verification failed!. Go back and try again.";
		}
				...
				if ($sEmail == "") {
					$sMessage		=	fn_Print_MSG_BOX("please enter your registered email address", "C_ERROR");
						...
						if (mysql_num_rows($rsPASSWORDREMIND)<=0) {
							...
							$ChnagePassword		=	mysql_real_escape_string($sUSER_PWD);
							$sSQL	=	"UPDATE tbl_user SET password=PASSWORD('$ChnagePassword') WHERE email = '$sEmail' ";
							...
							$sEmailMessage		.=	"<b>Your Password is :</b>".$sUSER_PWD."<br />";
						...
						<form name="frm1" action="passwordreminder.php" method="post">
							...
							<table cellpadding="0" cellspacing="5" border="0" width="350" align="center" class="box">
								...
								<tr>
									...
									<td width="250">
										<input type="text" name="txtemail" id="txtemail" value="" style="width:245px;"  />
									</td>
								</tr>
