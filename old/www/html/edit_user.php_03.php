		...
		$sPERMIT_SQL	=	"";
			...
			if ($bPictureErr==false){
				...
				if($_POST["supdatetype"]=='renew-driver-request'){
					...
					$sPERMIT_SQL	.=	", end_permit = '".$sEndPermit."' , reg_date = '".date('Y-m-d H:i:s')."', renew_date = '".$sRenwPermitDate."', renew_text = '".addslashes($_POST["txtrenewtext"])."', active = 0, new_user = 1, status_date = '".date('Y-m-d H:i:s')."'";
				}
				...
				if($_POST["supdatetype"]=='renew-permit'){
					//if(isset($_POST["optpermitexpireyear"]))	$sEndPermit			=	$_POST["optpermitexpireyear"]."-12-31";							
					$sPERMIT_SQL		.=	", end_permit = '".$sEndPermit."' , renew_date = '".$sRenwPermitDate."', renew_text = '".addslashes($_POST["txtrenewtext"])."', active = 1, new_user = 0, status_date = '".date('Y-m-d H:i:s')."'";
				}
				...
				if(isset($_POST["drpdepartment"]) && $_POST["drpdepartment"]!=""){
					...
					$sDEPT_SQL		=	", dept_id=	'". $_POST["drpdepartment"]."'";
				}
				if($bDEPT_STATUS==0){
				...
				if(isset($_POST["drpusergroup"]) && $_POST["drpusergroup"]!=""){
					$sGROUP_SQL_VAL	=	$_POST["drpusergroup"];
					...
					if($iUSER_PREV_GROUP!=$sGROUP_SQL_VAL){		//check if prev and this group are not same
						$sSQL	=	"SELECT user_id FROM tbl_user WHERE user_group = ".$sGROUP_SQL_VAL." AND LOWER(email) = '".strtolower($_POST["txtemail"])."'";
				...
				if($bUPDATION_ERROR==false){
					...
					$sSQL="UPDATE  tbl_user SET f_name='".$_POST["txtfname"]."', l_name='".$_POST["txtlname"]."' ".$sDEPT_SQL.", phone='".$_POST["txtphone"]."', ".
							...
							<table cellpadding="0" cellspacing="5" border="0" width="650" align="center" class="box">
								...
								<tr>
									...
									<td>
										...
										<?Php if(($_SESSION["User_Group"]==$iGROUP_TC || $_SESSION["User_Group"]==$iGROUP_TM) && ($_SESSION["User_ID"]!=$rowUSER['user_id'])){?>
											The Permit Expires Dec 31st of <br /><input type="radio" name="optpermitexpireyear" value="<?Php echo date('Y');?>" <?Php if(date('Y',strtotime($sPERMIT_EXP_YEAR))==date('Y')) echo "checked";?> /><span class="left">This year&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>&nbsp;<?Php //echo date('Y');?>
