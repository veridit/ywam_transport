<?
	...
	$sPIC_LINK			=	"";   $iMAX_PASSENGERS = 15;   $sLICENCE_COUNTRY = "";
	...
	if(isset($_POST["action"])	&& $_POST["action"]=="edituser"){
		...
		$NewPassword=	mysql_real_escape_string($_POST["txtpassword"]);
			...
			if ($bPictureErr==false){
				...
				if(isset($_POST["txtpermitdate"])) $sEndPermit = fn_DATE_TO_MYSQL($_POST["txtpermitdate"]);
				...
				if($bUPDATION_ERROR==false){
					...
					if($NewPassword == '') {
						...
						$passwordSQL = "password=PASSWORD('$NewPassword'), ";
					}

					$sSQL="UPDATE  tbl_user SET ".$passwordSQL."f_name='".mysql_real_escape_string($_POST["txtfname"])."', l_name='".mysql_real_escape_string($_POST["txtlname"])."' ".$sDEPT_SQL.", phone='".mysql_real_escape_string($_POST["txtphone"])."', ".
					"license_no = '".mysql_real_escape_string($_POST["txtlicenseno"])."', license_state = '".mysql_real_escape_string($_POST["txtlicensestate"])."', license_country = '".mysql_real_escape_string($_POST["txtlicensecountry"])."', max_passengers = '".mysql_real_escape_string($_POST["txtmaxpassengers"])."', license_expire = '".$sLicenseExpire."', drive_tested='".mysql_real_escape_string(addslashes($_POST["txtdrivetest"]))."', test_date='".$sTestDate."', home_st_country='".mysql_real_escape_string($_POST["txtcountry"])."', user_group=".$sGROUP_SQL_VAL.", user_type = '".mysql_real_escape_string($_POST["drpusertype"])."' ".
				  ...
				  $sSQL = "UPDATE tbl_user SET active = 0, status_date = '".date('Y-m-d H:i:s')."' WHERE `end_permit` < NOW()";
				  $rsACTIVE = mysql_query($sSQL) or die(mysql_error());
...
if($iUSER_ID!=0){
	...
	$sLICENCE_COUNTRY  	=	$rowUSER['license_country'];
	$iMAX_PASSENGERS		=	$rowUSER['max_passengers'];
	...
	$sPERMIT_EXP_YEAR		=	fn_cDateMySql($rowUSER['end_permit'],1);
...
if (frm.txtconfirmpassword.value != "" || frm.txtpassword.value != "") {
	...
	if (frm.txtlicensecountry.value==""){
		sErrMessage=sErrMessage+'<li class="bold-font">please enter your license country';
		iErrCounter++;
	}else{
		regExp = /[a-zA-Z0-9 \-]/i;
		if (!validate_field(frm.txtlicensecountry, regExp)){
			sErrMessage=sErrMessage+'<li class="bold-font">please enter valid license country';
			iErrCounter++;
		}
	}

	if (frm.txtmaxpassengers.value==""){
		sErrMessage=sErrMessage+'<li class="bold-font">please enter max allowed passengers';
		iErrCounter++;
	}else{
		regExp = /[0-9]/i;
		...
		if (!validate_field(frm.txtmaxpassengers, regExp) || maxPasengers < 5 || maxPasengers > 19){
			sErrMessage=sErrMessage+'<li class="bold-font">max allowed passengers must be in range 5-19';
			iErrCounter++;
		}
	}
						...
						<form name="frm1" action="edit_user.php" method="post">
							...
							<table cellpadding="0" cellspacing="5" border="0" width="650" align="center" class="box">
								...
								<tr>
									...
									<td width="300"><input <?Php echo $sREADONLY_FIELD;?> type="text" name="txtfname" value="<?Php echo $sFNAME;?>" maxlength="20" style="width:250px;"  /></td>
								</tr>
								<tr>
									<td class="label">Last Name:</td>
									<td><input <?Php echo $sREADONLY_FIELD;?> type="text" name="txtlname" value="<?Php echo $sLNAME;?>" maxlength="20" style="width:250px;"  /></td>
								</tr>
								<tr>
									<td class="label">Home Country:</td>
									<td><input <?Php echo $sREADONLY_FIELD;?> type="text" name="txtcountry" value="<?Php echo $sHOME_COUNTRY;?>" maxlength="30" style="width:100px;"  /></td>
								</tr>
								<tr>
								...
								<tr>
									...
									<?
										if($sREADONLY_FIELD==""){
											...
											echo "<input type='text' readonly name='txtbrithdate' value='".fn_cDateMySql($sDATE_OF_BIRTH,1)."' style='width:100px;' />";
										}
										?>
									</td>
								</tr>
								...
								<tr>
									<td class="label">License State/Province:</td>
								...
								<tr>
									...
									<td><input type="text" name="txtlicensecountry" id="txtlicensecountry" value="<?Php echo $sLICENCE_COUNTRY;?>" maxlength="60" style="width:250px;" /></td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td><input type="text" name="txtmaxpassengers" id="txtmaxpassengers" value="<?Php echo $iMAX_PASSENGERS;?>" maxlength="2" style="width:250px;" /></td>
								</tr>
								<tr>
								...
								<tr>
									<td class="label" valign="top">This permit expires:</td>
									<td>
										<div style="clear:both;"></div><span class="bold-font Highlight">
										<?Php if(($_SESSION["User_Group"]==$iGROUP_TC || $_SESSION["User_Group"]==$iGROUP_TM) && ($_SESSION["User_ID"]!=$rowUSER['user_id'])) { ?>
											<input type="text" name="txtpermitdate" id="txtpermitdate" value="<?Php echo $sPERMIT_EXP_YEAR; ?>" maxlength="10" style="width:100px;" class="date-pick dp-applied" />
											<?Php } else { echo $sPERMIT_EXP_YEAR; } ?>
											</span>
										</td>
								</tr>
								...
								<tr>
									<td class="label" valign="top">Group: <span class="Highlight">(assigned by TM)</span></td>
							...
							<?Php if(($_SESSION["User_Group"]==$iGROUP_TC || $_SESSION["User_Group"]==$iGROUP_TM) && ($_SESSION["User_ID"]!=$rowUSER['user_id'])){?>
							<tr>
								<td class="label">Allow Financial and <br /> Driver Reports</td>
								<td><input type="checkbox" name="chkDrvrPermission" value="1" <?Php if($bDRIVER_PERMISSION==1) echo "checked";?> onClick="fn_SET_DRVR_PERMISSION(this.checked);" /><span class="Highlight">(readonly)</span></td>
							</tr>
							<?	}?>
								...
								<tr>
										...
										<?	}elseif($_SESSION["User_Group"]==$iGROUP_TC || $_SESSION["User_Group"]==$iGROUP_TM){?>
                    						...
                    						<td colspan="2" align="center"><?	if($iUSER_ID!="782"){?> <input type="button" name="btnSUBMIT2" value="RENEW THIS DRIVER" class="Button" onClick="valid_user(this.form, 'renew-permit');" style="width:130px;" /><?	}?>
                      						&nbsp;&nbsp;&nbsp;
											...
											<?	if($rowUSER['active']==1 && $iUSER_ID!="782"){?><input type="button" name="btnInACTIVE" value="DEACTIVATE" class="Button" style="width:150px;" onClick="fn_ACTIVE_USER('inactive');" /><?	}	?>
											<?	$doit=0;if($rowUSER['active']==0 && $rowUSER['new_user']==0 && $doit==1){?><input type="button" name="btnInACTIVE" value="ACTIVATE" class="Button" style="width:180px;" onClick="fn_ACTIVE_USER('active-bad');" /><?	}	?>
										<?	}?>
<input type="button" name="btnDELETE" value="DELETE" class="Button" onClick="location.href='list_users.php'" style="width:110px;" title="Does not do anything yet."/>
