<?
	...
	//print(date('g:i a',strtotime('21:00:00')));
	if(!isset($_SESSION["User_ID"]) || $_SESSION["User_ID"]=="") header('location:login.php?err=1');
	
	//$sMessage			=	fn_Print_MSG_BOX("<li>if you didn't found a pending trip here, please also check abandoned trips", "C_SUCCESS");
	$sMessage			=	"";
	$iRESERVATION_ID	=	0;
	$iRECORD_COUNT		=	0;
	$iVEHICLE_ID		=	0;
			...
			if(mysql_num_rows($rsCHKDUPLICATE)>0){
				$sMessage		=	fn_Print_MSG_BOX("<li>trip already been closed", "C_ERROR");
					...
					if(strtotime(date('Y-m-d H:i:s'))> strtotime($sPLANNED_DEPART_DATETIME)){
						...
						"VALUES(".$iRESERVATION_ID.", ".$_POST["txtbeginmileage"].", ".$_POST["txtendmileage"].", '".$_POST["drpgas"]."', ".
						" ".$bProblem.", '".addslashes($_POST["txtproblem"])."', ".$iVEHICLE_MILE_COST.", ".$_SESSION["User_ID"].")";
						//print($sSQL);
						...
						if(isset($_POST["txtTMNotes"]) && $_POST["txtTMNotes"]!=""){	//add into comments table
							$sSQL="INSERT INTO  tbl_user_comments(posting_user_id, about_user_id, comments, trip_id) VALUES(".$_SESSION["User_ID"].", ".$sASSIGNED_DRIVER.", '".addslashes($_POST["txtTMNotes"])."', ".$iRESERVATION_ID.")";
			...
			if(mysql_num_rows($rsCHKDUPLICATE)>0){
				...
				if(mysql_num_rows($rsRETURN_CHK)>0){
					...
					if(intval($iSECONDS)<=0){
						...
						"VALUES('".addslashes($_POST["txtnotes"])."', ".$iRESERVATION_ID.", ".$_SESSION["User_ID"].", ".$iVEHICLE_MILE_COST.", ".$bCALCULATE_FINE.")";
						//print($sSQL);
						...
						if(isset($_POST["optmsgoption"]) && ($_POST["optmsgoption"]== "19" || $_POST["optmsgoption"]=="26")){		//if checkbox is selected then send an email to assigned driver
							...
							$sSQL			=	"SELECT MID(tbl_info_links.link_text, INSTR(tbl_info_links.link_text, '--SUBJECT--')+15, (INSTR(tbl_info_links.link_text, '--END SUBJECT--') -1)-(INSTR(tbl_info_links.link_text, '--SUBJECT--')+15)) FROM tbl_info_links WHERE link_id = ".$_POST["optmsgoption"];
							//print($sSQL);
							$sEmailSubject	=	str_replace('&nbsp;',' ', strip_tags(stripslashes(mysql_result(mysql_query($sSQL),0))));
							$sSQL			=	"SELECT MID(tbl_info_links.link_text, INSTR(tbl_info_links.link_text, '--MESSAGE BODY--')+20, (INSTR(tbl_info_links.link_text, '--END MESSAGE BODY--') -3)-(INSTR(tbl_info_links.link_text, '--MESSAGE BODY--')+20)) FROM tbl_info_links WHERE link_id = ".$_POST["optmsgoption"];
							//print($sSQL);
	...
	/*if(!isset($_POST["optcloseoption"]) && $iRESERVATION_ID!=0){
		...
		//print($sSQL);
		$rsRESERVATION	=	mysql_query($sSQL) or die(mysql_error());
		if(mysql_num_rows($rsRESERVATION)<=0){$sMessage	=		fn_Print_MSG_BOX("<li>Please select valid pending trip or trip has been closed or already abandoned", "C_ERROR");}
		else{
			$iRECORD_COUNT	=	mysql_num_rows($rsRESERVATION);
			$rowRESERVATION	=	mysql_fetch_array($rsRESERVATION);
			$iVEHICLE_ID	=	$rowRESERVATION['vehicle_id'];
...
function valid_trip(frm){
	...
	/*if(fn_CHECK_BIGN_MILES(frm)!=""){
		alert(fn_CHECK_BIGN_MILES(frm));
		fn_CHANGE_TEXT_BOX_COLOR(frm.txtbeginmileage, '#efc3df', '#ff0000');
	}*/
	
	/*sMileageErr = fn_CHECK_TRIP_MILEAGE(frm);
	if(sMileageErr!=""){
			sErrMessage=sErrMessage+'<li>please correct the mileage error';
			iErrCounter++;
			//frm.txtbeginmileage.style.backgroundColor	=	'#efc3df';
			fn_CHANGE_TEXT_BOX_COLOR(frm.txtbeginmileage, '#efc3df', '#ff0000');
			fn_CHANGE_TEXT_BOX_COLOR(frm.txtendmileage, '#efc3df', '#ff0000');
	}*/
	...
	/*if (iErrCounter >0 && sMileageErr!=""){
		
		fn_draw_ErrMsg(sErrMessage);
		fn_SHOW_POPUP(sMileageErr);
		
	}
	else if(iErrCounter >0 && sMileageErr==""){
		fn_draw_ErrMsg(sErrMessage);
	}else
		frm.submit();*/
...
function DisplayFormValues()
    {
        ...
        document.getElementById('lblelem').innerHTML = str;
    }
...
function fn_SAVE_OGM(frm){
	...
	if (iErrCounter >0){
		...
		$.get("ajax_data.php", {msgid: sMSG_ID, action: 'update-ogm', txtemail: tinyMCE.get('txtemail').getContent()}, function(data){			  	
	
				if (data=="ERROR"){
					$('#EmailMessage').html("Error to save changes in Message<br /> please contact with Web Admin");
				}else{
					$('#msg_box').hide();
					$('#Message').html(fn_draw_ErrMsg("<li>Changes have been saved to Message"));
					//$('#Message').html(fn_draw_ErrMsg("<li>"+data));
				}
		}, 'html');
	}
									...
									<table border="0" cellspacing="0" cellpadding="0" width="100%" height="100%">
								 		<tr>			
								  			<td class="TextObject" align="center">
								   				<h1 style="margin-bottom: 0px;">CLOSE TRIP SLIP</h1>
								  			</td>
								 		</tr>
									</table>
                   				</td>
				...
				<tr valign="top" align="left"><td colspan="4">&nbsp;</td></tr>
               	...
               	<tr valign="top" align="left">
                	...
                	<td width="949" class="TextObject" align="center">
						<form name="frm1" id="frm1" action="add_tripdetails.php" method="post">
										...
										<div id="close_box" <?Php if(!isset($_POST["optcloseoption"]) && $iRESERVATION_ID!=0) echo "style='display:block;'"; else echo "style='display:none;'";?>>
											...
											<tr>
												...
												<td>
													...
													<div id="assigned_driver" style="float:left; <? if($sASSIGNED_DRIVER!="") echo 'display:block'; else echo 'display:none;';?>"><span class="label left">Assgnd Driver:</span>&nbsp;<input readonly="" type="text" name="txtassigneddriver"  value="<? echo $sASSIGNED_DRIVER;?>" style="width:150px;"  /></div>
												</td>
											</tr>
											<tr>
									...
									<td colspan="2">
										<div id="abandon_box" style="display:none;">
											<table cellpadding="0" cellspacing="5" width="600">
												...
												<tr>
													<td width="150" class="label">Reservation:</td>
													<td width="450"><div id="abandon_pending_trips"></div><div id="lblelem"></div></td>
												</tr>
												<tr>
									...
									<td colspan="2">
										<div id="msg_box" style="width:650px; height:509px; display:none; z-index:5; top:270px; left:450px; position:fixed; background-color:#fff;">
											<table cellpadding="0" cellspacing="5" border="0" width="400" align="center" class="box" height="250">
												...
												<tr><td>&nbsp;</td></tr>
												<tr><td><!--<input type="button" name="btnSendEmail" value="SAVE CHANGES" class="Button" onClick="fn_SAVE_OGM(this.form);" />&nbsp;--><input type="button" name="btnCANCEL" value="CLOSE" class="Button" onClick="fn_HIDE_BOX();" /></td></tr>
											</table>
										</div>
									</td>
								</tr>
