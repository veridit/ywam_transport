...
By 		  Date		  Description
Claudiaf  13/10/2014  Qualified values in drpsort to reference correct table
					  (was causing errors on the f_name selection)
*/
	...
	if(isset($_POST["action"])	&& $_POST["action"]=="search"){
		...
		if(isset($_POST["drpdept"]) && $_POST["drpdept"]!="")			{
			$sDeptID	=	mysql_real_escape_string($_POST["drpdept"]);		$sCriteriaSQL	.=	" AND tbl_departments.dept_id = '".$sDeptID."'";
		...
		if(isset($_POST["drpcharge"]) && $_POST["drpcharge"]!="")			{$iCHARGE_VAL	= mysql_real_escape_string($_POST["drpcharge"]);		$sCriteriaSQL_CHARGES	=	" AND mileage.charges ".$iCHARGE_VAL;}
	//	if(isset($_POST["drpsort"]) && $_POST["drpsort"]!="")				{$sSORT_ORDER		=	$_POST["drpsort"];	}
		
		
		switch ($_POST["drpsort"]) {
		  case "assgnd ASC":
			$sSORT_ORDER = "tbl_user.f_name ASC";
			break;
		  case "assgnd DESC":
			$sSORT_ORDER = "tbl_user.f_name DESC";
			break;
		  case "planned ASC":
			$sSORT_ORDER = "planned_depart_day_time ASC";
			break;
		  case "planned DESC":
			$sSORT_ORDER = "planned_depart_day_time DESC";
		} 
											...
											<tr>
												...
												<td class="label" width="160">Sort By:<br />
													...
													<select name="drpsort" style="width:160px;" size="1">
														...
														<option value="assgnd ASC" <? if($sSORT_ORDER == "assgnd ASC") echo "selected";?>>Assgnd Driver A-Z</option>
														<option value="assgnd DESC" <? if($sSORT_ORDER == "assgnd DESC") echo "selected";?>>Assgnd Driver Z-A</option>	
														<option value="planned ASC" <? if($sSORT_ORDER == "planned ASC") echo "selected";?>>Depart Date # Ascending</option>
														<option value="planned DESC"  <? if($sSORT_ORDER == "planned DESC") echo "selected";?>>Depart Date # Descending</option>
													</select>
												</td>
