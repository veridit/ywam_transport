<?
	...
	if(mysql_num_rows($rsHOME_DEPT)>0){
		list($iHOME_DEPT_ID, $iHOME_DEPT_NAME)	=	mysql_fetch_row($rsHOME_DEPT);
	...
	if(isset($_POST["txtdestination"]) && $_POST["txtdestination"]!="")		$sDESTINATION	=		$_POST["txtdestination"];
	if(isset($_POST["txtpassenger"]) && $_POST["txtpassenger"]!="")			$iPASSENGER		=		$_POST["txtpassenger"];
	...
	$sTimePickerDate1=date('m/d/Y',strtotime(date("Y-m-d")." +1 day"));
						...
						if($bONLY_2_3_VEHICLES	==	true){
							...
							if($bREPEAT_RESERVATION	==	true){//there would be repeating reservation calculation==================
								...
								for($iWeekCounter=1;$iWeekCounter<=intval($_POST["drpweek"]);$iWeekCounter++){
									...
									if($iDAYS_LIMIT>=59)
										$sMessage	=	fn_Print_MSG_BOX("<li class='bold-font'>Reservations more than 60 days in the future are not allowed", "C_ERROR");
									else{
								...
								<tr>
									<td align="center">
										<table cellpadding="0" cellspacing="5" border="0">											
											<tr>
												<td class="label" width="150">Assgnd Driver:</td>
												<td colspan="2">
												<?Php 
													...
													?>
													<span class="Highlight bold-font">Our insurance only covers drivers registered in this system</span>
												</td>
											...
											<tr>
												...
												$sSQL	=	"SELECT v.vehicle_id, v.vehicle_no, v.passenger_cap FROM tbl_vehicles v WHERE v.vehicle_id NOT IN (SELECT s.vehicle_id FROM tbl_srvc_resvs s WHERE s.is_cancelled = 0) AND v.sold = 0 ORDER BY (vehicle_no+0)";
											...
											<tr>
												...
												<td colspan="2"><?	fn_DEPARTMENT("drpdept", $sBILLING_DEPT, "200", "1", "--Select Billing Dept--");?><span class="Highlight bold-font">Do not charge another dept. without approval</span></td>
												<?Php	}?>
											</tr>
