...
if [[ ! -f "tmp/transport-dump.sql" || $(find "tmp/transport-dump.sql" -mtime +1) ]]; then
  ...
  echo "Using existing mysql data"
fi
...
# Wait until MySQL has at least a two-digit number of tables
count=0
...
echo "Migrate from MySQL to PostgreSQL"
...
./devops/view-schema.sh > doc/schema-old.md
